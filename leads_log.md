...
## Run: 2026-07-08 17:11 (DEEP BACKFILL — 30 day window)

**THIS IS A ONE-TIME DEEP BACKFILL, NOT A DAILY PASS.** Run 2026-07-08, 30-day
window (~Jun 8 – Jul 8). Future daily runs should keep using 24h and treat
everything below as already-seen. Read `leads_log.md` in FULL first and de-duped
against ALL twelve prior runs — **including the 07-03 → 07-07 backfills, which
used this same 30-day window and the same sources within days of this one.** As
expected for a 7th same-window pass in ~6 days, the large majority of what the
trackers surfaced was already logged; **only genuinely net-new leads (not in any
prior run, by URL and by company/role) appear below** — a deliberately short,
honest list, not a padded one. Dedup was done programmatically (extracted all 369
prior URLs + 147 jobright IDs + all logged company names from the full log, then
filtered every candidate against them) AND by prose-mention grep for the shortlist.

**Coverage summary (fully / partially / not at all):**
- **FULLY COVERED (curl'd raw markdown from `raw.githubusercontent.com`, all HTTP
  200, parsed locally with Python — the proven method; NOT WebFetch):**
  - `zapplyjobs/New-Grad-Data-Science-Positions` (484 lines) — read in full;
    richest source again. Carries a Visa column (`🏛 H-1B Co.` / `✅ Sponsor` /
    blank) = **company-level** sponsorship signal (generally OPT-friendly), NOT a
    per-JD guarantee. 59 target-title/non-senior/in-window rows after parse; after
    non-defense + non-FAANG + dedup, ~4 net-new (Red Hat, Fastly + already-logged).
  - `jobright-ai/2026-Engineering-New-Grad` (`master` branch, 5298 lines) — read in
    full; calendar-dated. In-window target rows span **Jul 01 – Jul 08**. 55 parsed;
    most were TikTok/SpaceX/Tesla (FAANG-adjacent/senior) or already-logged.
  - `jobright-ai/2026-Data-Analysis-New-Grad` (`master`, 315 lines) — read in full,
    still **caps at "last ~7 days" (Jul 01 – Jul 08)** per its capacity constraint;
    30-day depth unavailable (same cap every prior run).
  - `jobright-ai/2026-Business-Analyst-New-Grad` (`master`, 114 lines) — read in
    full; **0** target-title in-window rows after dedup (thin, as every prior run).
  - `speedyapply/2026-AI-College-Jobs` → `NEW_GRAD_USA.md` (379 lines) — read in
    full. **No longer dead this run** — yielded the best net-new explicit-"I" entry
    rows (Uber DA I, Lurie Children's DE I, Snap Finance DA I, Flashpoint DE I,
    Handshake MLE I). 27 in-window (≤30d) target rows parsed; ~5 net-new after dedup.
    (Contrast 07-07 which found 0 — the age window rolled forward and surfaced fresh
    rows.) NOTE the two jobright ENG repos and BA repo are on `master`, not `main`
    (my first `main` fetch 404'd — flagged for future runs).
- **PARTIALLY COVERED (WebSearch works; underlying JD pages 403 → bodies /
  seniority / exact post date / visa language UNVERIFIED):**
  - **YC Work-at-a-Startup** — `WebSearch` over `site:ycombinator.com/companies …
    "Forward Deployed Engineer"` and `… "Applied AI Engineer"` returned real direct
    `ycombinator.com/companies/.../jobs/` URLs (the one reliable community channel,
    every run). Net-new FDE cluster logged in the Community section. The Applied-AI
    query returned **zero** net-new (all — Maywood, Model ML, Delve, PermitFlow,
    Firstbase, Artisan, + Founding-excluded Bild/Saphira/Kastle — already logged).
- **NOT COVERED / BLOCKED (unchanged structural blockers, re-confirmed this run):**
  - **Individual JD verification — ALL still blocked.** `curl` to greenhouse /
    ashby / workday / oracle / smartrecruiters / jobright = **HTTP 000 (egress
    tunnel denied)**; `WebFetch` to the same = **403** (every prior run). **No JD
    body read** → EVERY visa bucket below is inferred from company type + role (+
    zapply's H-1B column where present), NOT JD text. Every "verify" must be opened
    manually. Seniority for any role not explicitly labeled entry/junior/associate/
    "I"/"1"/new-grad is **unverified** and flagged inline.
  - **HN "Ask HN: Who is hiring? (July 2026)" thread `48747976`** — re-tested this
    run: `hn.algolia.com/api/v1/items/48747976` = **000**, `hn.algolia.com/api/v1/
    search` = **000** (egress-denied). Same blocker as every prior run. Nothing from
    HN logged. Remains the #1 un-mined community gap.
  - **Reddit / Discord / Slack** — not machine-readable (hard-blocked every prior
    run). No active datable "who's hiring" thread surfaced with per-post links.
  - **LinkedIn / Indeed / Glassdoor / Wellfound / Built In** — only aggregator
    landing pages, no per-posting dates or stable direct links → nothing logged. No
    founder/recruiter post with a stable direct link surfaced this run.
- **Recency basis:** jobright rows = calendar dates (Jul 01 – Jul 08, used
  directly). speedyapply = "age" field in days (17d/22d/9d/13d/1d) read against the
  Jul-8 refresh — treated as first-seen-age; all comfortably inside the 30-day
  window. zapply minute/hour stamps ("54m") are scraper FIRST-SEEN, not confirmed
  post dates, but inside window. YC community rows have **no verifiable post date**
  (JD 403) → flagged recency-unverified.
- **De-duped OUT as already logged** (surfaced again this run, NOT repeated below):
  UT Austin (Data Engineer I — logged 07-05), Nasdaq (Investigative DS/AI Eng —
  excluded 07-05), Gen Digital (MLE I), iFIT (Jr Salesforce AI Eng), Rolls-Royce
  (Jr MLE), Capgemini (Jr DE), UST / UST HealthProof (Assoc DE), Tata (Jr DE),
  NewsBreak (Applied AI Eng), Unity (MLE), CNN (MLE I Atlanta), Together AI
  (Analytics Eng), MeritFirst (AI Eng), Lincoln Financial (Actuarial DE), PayPal
  (MLE ×2 Austin), Phenom (Assoc DE), BD (Commercial Analytics Eng), HPE (DS),
  Travelers (DE I Richardson TX), GlobalFoundries (NCG DE), SBT Global (Jr DA),
  A-TEK (Health IT DA I), PTC (Jr DA/Analytics Eng), Vanguard/Elevance/State
  Street/ICF/Caterpillar/Blueprint/Volvo/AST/Core Specialty/Senseye/Civic
  Nation/CharterUP/Harris (all prior).

### Tier 1 — Hot & strong fit (net-new)
_Explicit-entry + on-target-title + in-window + commercial (non-defense, non-FAANG) + strong applied-AI/data fit. Visa inferred (JDs all 403/000)._

| Company | What they do / size | Role | Direct link | Posted (basis) | Visa posture | Fit note | Source |
|---|---|---|---|---|---|---|---|
| Handshake | Early-career recruiting / campus-hiring marketplace; large growth-stage (private, ~$3.5B val) | Machine Learning Engineer I | https://jobs.ashbyhq.com/handshake/dd1fd4fc-6f6d-4a3d-8c54-dd91ed48dd70 | speedyapply 22d first-seen (≈Jun 16) | Silent, growth startup — likely OK | **Explicit "I" entry MLE at a growth-stage AI-invested product co** — the single strongest net-new applied-ML fit this run; SF; recsys/matching + LLM work maps to the candidate's ship-real-systems profile; gap: SF onsite, recsys-leaning vs RAG/LLM, seniority-verify (JD 403) | speedyapply |

### Tier 2 — Worth a look (net-new; caveat = seniority and/or visa unverified, JD 403/000)

| Company | What they do / size | Role | Direct link | Posted (basis) | Visa posture | Fit note | Source |
|---|---|---|---|---|---|---|---|
| Uber | Ride-hail / delivery / mobility platform; very large public (non-FAANG, in scope) | Data Analyst I - Tech | https://www.uber.com/global/en/careers/list/160033 | speedyapply 17d first-seen (≈Jun 21) | Silent, larger company — **verify** | **Explicit "I" entry DA** ($173k/yr listed) at a large in-scope tech co; SF; SQL/analytics/experimentation fit; gap: large-co immigration policy unread, DA not applied-AI-eng | speedyapply |
| Snap Finance | Consumer point-of-sale lending / lease-to-own fintech; mid-large private | Data Analyst I | https://snapfinance.wd1.myworkdayjobs.com/en-US/snap_external_careers/job/Metro-9---West-Valley-City/Data-Analyst-I_R-2803 | speedyapply 9d first-seen (≈Jun 29) | Silent, mid-large — likely OK | **Explicit "I" entry DA** at a consumer-fintech; West Valley City UT; SQL/BI fit; gap: seniority-verify, DA-not-eng, fintech-analytics niche | speedyapply |
| Ann & Robert H. Lurie Children's Hospital of Chicago | Pediatric academic medical center; large (hospital, cap-exempt) | Data Engineer I | https://luriechildrens.wd1.myworkdayjobs.com/en-US/externalportal/job/Streeterville-Chicago-IL/Data-Engineer-I_JR2026-1951 | speedyapply 1d first-seen (≈Jul 7) | Likely OPT-friendly (hospitals/universities are cap-exempt & routinely hire OPT), **verify** | **Explicit "I" entry DE** at a cap-exempt employer (lower visa risk); Chicago IL; dbt/SQL/ETL fit; gap: healthcare-data domain, onsite | speedyapply |
| Flashpoint | Commercial cyber threat-intelligence / risk-data platform; mid-large private | Data Engineer I | https://jobs.ashbyhq.com/flashpoint.io/84e64d83-f0b2-43db-8151-94dc75f2c705 | speedyapply 13d first-seen (≈Jun 25) | Silent, mid-large — **verify** | **Explicit "I" entry DE + Remote-US**; dbt/SQL/Airflow fit; gap: threat-intel domain (commercial, not gov, but data-sensitive — verify no US-person clause), seniority-verify | speedyapply |
| Red Hat | Enterprise open-source software (IBM subsidiary); large (non-FAANG) | Machine Learning Engineer | https://redhat.wd5.myworkdayjobs.com/jobs/job/Boston/Machine-Learning-Engineer_R-058400 | zapply 54m first-seen (in-window; scraper stamp) | 🏛 H-1B Co. (zapply) → likely OPT-OK, **verify** | MLE target title at a major open-source shop; Boston; strong applied-ML/infra fit; gap: **no entry marker → seniority unverified**, large-co policy | zapplyjobs |
| Fastly | Edge-cloud / CDN / compute platform; large public (non-FAANG) | Data Engineer | https://www.fastly.com/about/jobs/apply?gh_jid=8048891 | zapply 54m first-seen (in-window; scraper stamp) | ✅ Sponsor (zapply) → likely OPT-OK, **verify** | DE target at an infra co that **explicitly sponsors**; SF; dbt/SQL/Airflow fit; gap: no entry marker → seniority unverified | zapplyjobs |
| Burford Capital | Litigation-finance / legal-asset investment firm; mid-large public | AI Engineer | https://jobright.ai/jobs/info/6a0a86c9a203b1052e44da9a | Jul 07 (jobright) | Silent, larger company — **verify** | **AI Engineer target title** at a finance firm building internal AI; NYC; applied-AI fit; gap: no entry marker → seniority unverified, onsite NYC, finance domain | jobright ENG |
| Astrana Health | Value-based / physician-network healthcare-delivery tech; large public (non-FAANG) | Forward Deployed Engineer | https://jobright.ai/jobs/info/6a4df512c643fd23fed3e387 | Jul 07 (jobright) | Silent, larger company — **verify** | **FDE target title** at a health-tech co; Monterey Park CA (hybrid); customer-facing deploy fit; gap: no entry marker → seniority unverified, healthcare domain | jobright ENG |
| Northwell Health | Largest NY health system / hospital network; very large (non-profit health) | Associate Business Intelligence Analyst (Onsite) | https://jobright.ai/jobs/info/6a4d272ed27b2c4dda9b6672 | Jul 07 (jobright) | Likely OPT-friendly (non-profit hospital, cap-exempt), **verify** | **Explicit "Associate" BI Analyst** = entry rung + BI target title; Staten Island NY; SQL/BI/reporting fit; gap: onsite-only, healthcare-BI niche | jobright DA |
| NetSuite (Oracle) | Cloud ERP / business-software suite (Oracle-owned); large (non-FAANG) | Machine Learning Engineer (Remote) | https://jobright.ai/jobs/info/6a4d245bd27b2c4dda9b658d | Jul 07 (jobright) | Silent, larger company — **verify** | MLE target + **Remote-US**; applied-ML fit; gap: no entry marker → seniority unverified, Oracle-scale immigration policy | jobright ENG |

### Tier 3 — Long shots / needs verification / seniority or domain risk (net-new)

| Company | What they do | Role | Direct link | Posted | Visa posture | Why Tier 3 | Source |
|---|---|---|---|---|---|---|---|
| Bechtel Corporation | Engineering / construction / project-management firm; very large private | Data Scientist / AI Engineer | https://jobright.ai/jobs/info/6a4dd908c643fd23fed3e0fb | Jul 07 (jobright) | Unclear — **verify** | AI-Engineer target title, Reston VA; **CAVEAT: Bechtel runs large federal/gov & nuclear projects → some roles carry US-person/clearance requirements** even though the company is commercial; verify before effort; seniority unverified | jobright ENG |
| Virtusa | IT-services / digital-consulting firm; large | Associate Engineer - Technology (AI/ML) | https://jobright.ai/jobs/info/6a4d77f9d27b2c4dda9b7bf3 | Jul 07 (jobright) | Silent, larger company — **verify** | **Explicit "Associate" + AI/ML**; NYC; applied-ML fit; gap: **IT-services consultancy may staff client accounts / bench** (scope risk), verify the role is a real product-team hire | jobright ENG |
| CodeVertex Innovations | Small/opaque IT-services vendor; small | Entry-Level Data Analyst | https://jobright.ai/jobs/info/6a4dff391544d7246c0d188d | Jul 08 (jobright) | Unclear — **verify** | **Explicit "Entry-Level" + Remote-US** DA; SQL/BI fit; gap: opaque small vendor → **verify legitimacy/scope before effort** (staffing-shop risk, same pattern as AgileGrid flagged 07-06) | jobright DA |
| Leica Biosystems (Danaher) | Cancer-diagnostics / pathology instruments (Danaher operating co); large | Data Analyst, Global Trade Compliance | https://jobright.ai/jobs/info/6a4d52040209ea6fd6851c6f | Jul 07 (jobright) | Silent, larger company — **verify** | DA target, Washington DC (remote); **CAVEAT: almost certainly the SAME req as "Danaher – Data Analyst, Global Trade Compliance (DC)" logged 07-07** (Leica Biosystems is a Danaher company) — treat as a likely dup; trade-compliance niche + seniority unverified | jobright DA |

### Community / informal leads (YC Work-at-a-Startup — real direct links; JD pages 403 → seniority/visa/post-date UNVERIFIED)
_All are YC seed/early startups → visa: silent, small startup (OPT needs no sponsorship, generally OK; tiny startups sometimes can't accommodate — verify). Open each to confirm entry-level + US-eligibility before applying. Net-new only (FurtherAI, Aravolta, Cekura, Luminai, Maywood, Model ML, Delve, PermitFlow, Firstbase, Artisan, CollectWise + Founding-excluded Paragon/Bild/Saphira/Kastle already logged/excluded in prior runs)._
- **Cedana** — Forward Deployed Engineer: AI + HPC (checkpoint/restore & GPU-workload orchestration; customer-facing technical engagement). https://www.ycombinator.com/companies/cedana/jobs/d1vYocG-forward-deployed-engineer-ai-hpc — FDE target title; infra/HPC-heavy (systems-leaning vs applied-GenAI). Seniority unverified.
- **Mercura** — Forward Deployed Engineer (FDE): "the technical person who turns a signed deal into a live, working AI deployment." https://www.ycombinator.com/companies/mercura/jobs/UihXnwP-forward-deployed-engineer-fde — clean commercial-FDE fit (deploy AI for customers). Seniority unverified.
- **Trellis AI** — Forward Deployed Engineer (full-time): build/deploy computer-use agents for patients to access life-saving medicine (processes billions in therapies). https://www.ycombinator.com/companies/trellis-ai/jobs/f4GWvH0-forward-deployed-engineer-full-time — **explicit full-time** + agentic-AI deploy = strong fit to the candidate's LangGraph/agent work. Seniority unverified.
- **HappyRobot** — Forward Deployed Engineer: customer-facing technical role helping customers maximize an AI (logistics-voice-agent) platform. https://www.ycombinator.com/companies/happyrobot/jobs/atsn2pi-forward-deployed-engineer — FDE target title at a funded AI-agents startup. Seniority unverified.
- **EXCLUDED (seniority/founding this run):** Paragon (*Founding* FDE), Bild AI (*Founding* Applied-AI), Saphira AI (*Founding*, 5+ yrs), Kastle (*Founding* Applied-AI).

### Intern / Co-op & Part-time (in-scope per brief, but timing-flagged for a May-2026 grad → OPT starts Jul 13 2026)
_Prompt lists Intern/Contract as in-scope. Listed for completeness, not recommended over FTE — a Fall-2026 intern/co-op or part-time residency conflicts with a new-grad FTE search and consumes OPT time on a non-FTE rung._
| Company | Role | Direct link | Posted | Visa | Note |
|---|---|---|---|---|---|
| d-Matrix | Applied AI Engineering Intern | https://jobs.ashbyhq.com/d-matrix/93a6e6ee-3391-4437-8459-e28eb05eace7/application | zapply 1mo first-seen | ✅ Sponsor | AI-chip startup, Santa Clara; **Applied-AI target + sponsors**, but intern rung |
| Aptura | Associate – AI Residency (Applied AI x Corporate Finance) — **Part-time** (5 near-identical IB/PE/M&A variants) | https://jobs.ashbyhq.com/aptura/db948822-50bd-483e-ab3f-32aeb84ba5e2 | speedyapply 8d first-seen | (blank) | NYC; applied-AI-in-finance residency but **part-time** — not an FTE path |

### Recency note
All tiered jobright rows carry calendar post dates (Jul 07 – Jul 08). speedyapply rows carry day-age first-seen stamps (1d–22d → ≈Jun 16 – Jul 7), all inside the 30-day window. Red Hat/Fastly carry zapply minute first-seen stamps (54m, inside window, scraper-first-seen not a confirmed post date — flagged inline). YC community rows have no verifiable post date (JD 403) → recency-unverified. No separate "Recency unverified" tier needed — every net-new tiered lead is datable within the window except the YC community rows (flagged).

### Excluded (net-new sightings this run, logged so future runs don't re-surface)
- **Defense / federal / clearance / gov (US-person near-certain):** Booz Allen Hamilton (Data Engineer DC/Honolulu, AWS Streaming DE Springfield, Cybersecurity AI/ML Eng McLean, Computer Vision AI Eng McLean), CACI (Data Analyst DC, JIATF-401 DE Alexandria, Data-Access-Layer DE ×2 Remote), KBR (DE Chantilly), GDIT (Agentic AI Engineer Falls Church), Guidehouse (Business Ops & Data Analyst McLean, Junior DE McLean — gov consulting), Peraton (Jr Cloud SWE AI/ML TS/SCI Annapolis Junction, Jr Software DE Fort Meade), Captivation Software (SWE 1 AI/ML Annapolis Junction — NSA-adjacent), Radiance Technologies (Jr DA — defense, excluded prior), ITA-International (Jr DA — maritime/defense-services contractor), Anduril Industries (Jr People Data Analyst Boston — weapons/defense → US-person), Tocaro Blue (Data Analyst Pensacola — maritime-domain-awareness, defense-adjacent), Innovative Defense Technologies (Associate AI Engineer Mount Laurel/Arlington), California Correctional Health Care Services (Research Data Analyst I / Data Analyst — state corrections), Bechtel noted in Tier 3 (verify, not hard-excluded).
- **FAANG-tier / near-FAANG / trading-floor (referral or out-of-scope):** Google (Cloud Data & AI Eng, Applied AI Eng Audio/XR), Amazon/AWS (Applied Scientist AWS Automated Reasoning, Systems Dev Eng AI/ML Dedicated Cloud, Data Engineer Ads, BI Engineer I SCOT Toronto, ICQA DA), Apple (MLE SIML / Health AIML / Foundation Models, Applied AI Eng), Meta (Data Engineer Product Analytics University Grad), TikTok/ByteDance (numerous MLE/Applied-Scientist Graduate — 2026/2027 Start, mostly PhD), SpaceX (AI Eng Platform Infra Special Programs, ML Eng Surrogate Modeling — Special Programs/ITAR-adjacent), Tesla (AI Eng Optimus Whole-Body-Controls), Jane Street (MLE — prop-trading/quant), Salesforce (DE MTS/SMTS/LMTS — level ambiguous, large), Intel (MLE Santa Clara — seen prior, DTCO DE Intern), Adobe (MLE — seen prior).
- **Non-US geography (F-1 OPT is US-only):** Baker Hughes (AI Engineer — Haryana India), Flight Centre Travel Group (Jr DE — Toronto ON ×2), Lyft (Data Analyst — Toronto), Ericsson (BI co-op — Ottawa), FCC Construcción (DA Co-op — Toronto), Veolia (DE — Paramus NJ is US but its EU parent SmartRecruiters posting; kept for verify only), Baltic Apprenticeships (DA Apprentice — UK), Islington Council / Flight Centre (UK/Canada).
- **Co-op / intern / part-time / student / clinical / research (weak timing or non-fit):** Campbell Soup (Data Engineer DA&AI Co-Op ×3, Agentic AI Engineer Co-Op ×2), Intel (DTCO DE Intern Austin), TikTok/ByteDance (MLE Intern Seattle, Applied Scientist Intern), Volvo Group (BI Fall-2026), Ensign College (Student-Employee DA), University of Pennsylvania (AI Solutions Specialist — **Temporary**), Beth Israel Lahey (Healthcare DA — OBGYN/clinical), Senseye/Siemens (Research DA), Capital One (Data Analyst Associate 2027 / Data Analyst Intern Summer 2027 — start too far out), Aptura (5× AI-Residency **part-time**, noted in Intern section).

### Bonus — adjacent titles worth adding to the search vocabulary
- **Explicit "I" entry titles are the highest-signal net-new source this run** (Uber DA I, Lurie Children's DE I, Snap Finance DA I, Flashpoint DE I, Handshake MLE I) — the `"<Title> I"` / `"<Title> 1"` suffix on speedyapply reliably marks the entry rung when a JD can't be read; keep parsing for it explicitly.
- **Cap-exempt employers = lower visa risk (hospitals + universities):** Lurie Children's (DE I), Northwell Health (Assoc BI Analyst) — non-profit hospitals and universities are H-1B cap-exempt and routinely hire OPT, a materially lower-risk visa bucket than for-profit large-cos. Weight these up.
- **"Associate Business Intelligence Analyst" (Northwell)** — the BI-Analyst/BI-Developer target behind an explicit "Associate" entry marker at a health system.
- **YC commercial-FDE cluster keeps refilling** (net-new this run: Cedana, Mercura, Trellis AI, HappyRobot) — the `site:ycombinator.com/companies "Forward Deployed Engineer"` query remains the single most productive community channel; the Applied-AI variant is now fully-mined (0 net-new this run).
- **TX in-state net this run:** none net-new (UT Austin DE I already logged 07-05; Travelers DE I Richardson already logged). Fidelity Westlake / PayPal Austin / Tata Plano remain the standing TX-metro options from prior runs.

### TODO for next (daily) run
- **CONCURRENCY:** this is the 7th backfill on `main` in ~6 days (07-03 → 07-08). Always
  `git fetch origin main` and read the LATEST log before de-duping — same-window
  one-day-apart passes over the same sources are ~90%+ overlap; expect a short net-new
  list and DO NOT pad it. Tier 1 was a single lead this run.
- **REPO BRANCHES:** `jobright-ai/2026-Engineering-New-Grad`, `-Data-Analysis-New-Grad`,
  and `-Business-Analyst-New-Grad` are on **`master`**, not `main` (a `main` fetch 404s).
  `speedyapply` + `zapplyjobs` are on `main`. Hard-code the right branch per repo to
  avoid a wasted 404 round-trip.
- **Source health:** `speedyapply/NEW_GRAD_USA` **revived this run** (best explicit-"I"
  net-new after several dead runs — the age window rolls forward, so keep checking it,
  don't demote permanently). `zapply` + `jobright ENG` remain the workhorses (jobright
  DA caps at ~7 days; jobright BA still 0). Keep **curl + local Python parse, NOT
  WebFetch** (all HTTP 200).
- **HN July thread `48747976`** still 000 on every machine-read path (Algolia items +
  search) — re-confirmed. Reddit/Discord still hard-blocked. Highest-value unsolved
  community item; YC WebSearch remains the only working community channel.
- **JD/visa verification** unchanged #1 structural blocker: greenhouse/ashby/workday/
  oracle/smartrecruiters/jobright = 403 (WebFetch) / 000 (curl egress-denied). All visa
  buckets inferred; zapply's H-1B/Sponsor column is the only per-company signal.
...
## Run: 2026-07-11 17:12 (DEEP BACKFILL — 30 day window)

**This is a one-time DEEP BACKFILL, not the daily 24h pass** — window is the last **30 days (≈Jun 11 – Jul 11 2026)**. This is the 8th backfill on `main` in ~8 days (07-03 → 07-08 → 07-11), so overlap with prior runs is very high; the net-new list below is deliberately short and NOT padded. Every lead is cross-checked against the full prior log (company + role); exact repeats are dropped.

### Coverage & honesty notes (read before trusting anything below)
- **Sources covered FULLY (machine-read, HTTP 200 via curl + local Python parse):** `jobright-ai/2026-Engineering-New-Grad` (master), `jobright-ai/2026-Data-Analysis-New-Grad` (master), `jobright-ai/2026-Business-Analyst-New-Grad` (master — 0 in-scope net-new), `speedyapply/2026-AI-College-Jobs` (main), `speedyapply/2026-SWE-College-Jobs` (main — 0 net-new in-scope). YC Work-at-a-Startup via WebSearch (`site:ycombinator.com/companies` FDE + AI-Engineer queries).
- **Sources NOT covered / BLOCKED this run (unchanged structural blockers, re-confirmed):**
  - **Individual JD verification — STILL 403 on everything.** Re-tested live this run: `WebFetch` to jobright.ai (Quora req) = **403**, `WebFetch` to ycombinator.com (Klarity req) = **403**. `curl` egress = **000**. **No JD body read** → EVERY visa bucket below is inferred from company type + role, NOT JD text. Every "verify" must be opened manually.
  - **HN "Ask HN: Who is hiring? (July 2026)" thread `48747976`** — re-tested this run on FOUR paths: `news.ycombinator.com/item` = **403**, `hnhiring.com/search` = **403**, `nchelluri.github.io/hnjobs` = **403**, `dheerajck.github.io/hnwhoishiring` = **000**. HN remains the #1 un-mined community gap — machine-unreadable on every path tried across all runs. Two named leads leaked via WebSearch *snippets* (no clickable per-post link retrievable): **Prolific — AI Solutions Engineer** (applied automation / ML / agentic; on-target title, link NOT captured) and **Cardog** (Toronto, Canada → non-US, excluded).
  - **speedyapply `NEW_GRAD_USA` standalone repo** — 404 this run (was the best explicit-"I" source on 07-08). It appears to have moved to a file INSIDE `2026-AI-College-Jobs/NEW_GRAD_USA.md` (per a WebSearch hit) rather than its own repo; the `2026-AI-College-Jobs/README.md` I parsed is mostly intern/PhD rows. **Flag for next run:** try `raw.githubusercontent.com/speedyapply/2026-AI-College-Jobs/main/NEW_GRAD_USA.md` directly.
  - **Reddit / Discord / Slack** — not machine-readable, hard-blocked every prior run; not re-tested (no working path exists). No datable "who's hiring" thread with per-post links.
  - **LinkedIn / Indeed / Glassdoor / Wellfound / Built In** — only aggregator landing pages, no per-posting dates/stable links → nothing logged. No founder/recruiter post with a stable direct link surfaced.
- **Recency basis:** jobright rows carry calendar post dates (used directly, all Jul 04–Jul 11, inside window). speedyapply rows carry day-age first-seen stamps. YC community rows have **no verifiable post date** (JD 403) → recency-unverified, flagged inline.
- **De-duped OUT as already logged** (surfaced again this run with the SAME role, NOT repeated below): Innodata (Applied AI Associate), Chime (AI/ML Engineer), CRISP DC (Data Engineer Associate), Quantifind (Assoc DS), Gravie (Data Analyst), Liberty University (Business Data Analyst I), AssetMark (Associate Data Engineer), CyberProof/UST (Assoc DE Early-Career — Bellevue/Frisco/Atlanta variants), AEG (Assoc DS), Together AI (Analytics Eng), Astrana Health (FDE), Northwell (BI Analyst), Danaher (DA Global Trade Compliance), PTC (Jr DA/Analytics Eng), Tocaro Blue (DA), Core Specialty (Actuarial DA), AST (BI Analyst), CharterUP (DA), California Correctional (Research DA I), Capital One (DA Associate 2027 / DA Intern 2027), U Penn (AI Solutions Specialist — Temporary), Ensign College (Student DA), HackerRank (FDE — same YC link), StackAI (Forward Deployed AI Eng — same YC link), FurtherAI/Mercura/Cekura/Paragon (YC, logged). WaFd Bank BI was **explicitly "kept out" in a prior run** (seniority-unverified) — not re-surfaced.
- **Excluded by hard filter this run (logged so future runs don't re-surface):**
  - *FAANG / near-FAANG / referral-channel:* Amazon (Applied Scientist Brand Protection; DS I SCOT; ICQA DA), TikTok/ByteDance (numerous MLE/Applied-Scientist Graduate — near-FAANG, referral), Tesla (AI Eng Optimus), SpaceX (AI Eng Special Programs / ML Surrogate Modeling / Starlink DE — ITAR), Figma (DS Core Data — PhD).
  - *Defense / federal / clearance / gov (US-person near-certain):* Booz Allen (DE Honolulu ×2, DS ×8 Fort Meade/Stafford/Atlanta), Peraton (Jr Software DE Fort Meade), AMERICAN SYSTEMS (DE Jr Quantico), Northrop Grumman (Assoc DS Palmdale), CACI (DS Norco), General Dynamics/GDIT (Human Performance DS I Hurlburt Field), MANTECH (Jr DS Sterling), Constellis (DA Philadelphia), Command Holdings (DA — *Cleared*, El Paso), Technomics (Cyber Risk DE Arlington), Clarity Innovations (DE MacDill AFB), DCCA (MLE Littleton), Morgan Business Consulting (Jr DS Arlington), Lentech (Jr DS Fort Meade), Mission Technologies/HII (BI Dev "Data Scientist 2" Fairfax), Elbit Systems of America (Ops DA Roanoke — defense mfg), **The Nuclear Company (AI Engineer 1 — DC; nuclear-energy build, clearance/US-person risk → moved to exclude)**, Sitreps 2 Steercos (FDE — gov/defense-adjacent naming).
  - *Trading-floor / prop-quant:* Jump Trading (Campus AI Research Eng — Deep Learning), Point72 (DS Proprietary Research).
  - *Non-US geography (F-1 OPT is US-only):* Prolific (AI-Training roles — Oxford UK), TrendAI (Applied AI Jr Web Dev — Ottawa), Cardog (Toronto), ING (parent posting Amsterdam — but a US Waco-TX variant exists, see Intern section).
  - *Co-op / intern / temp / student / research-clinical / niche-QA (weak timing or non-fit):* SharkNinja (SharkByte Applied AI Co-op Fall-2026), Campbell Soup (Agentic AI Eng Co-Op ×2, DE DA&AI Co-Op — logged), Intel (DTCO DE Intern Austin — logged), Michael Baker (Pavement DA — *Temp*), U Penn (AI Solutions Specialist — *Temporary*, logged), Ensign College (Student DA, logged), Northwestern (Research DA Associate ×2 — clinical/infectious-disease), Mass General Brigham (Research DA per-diem; Clinical Research Coord/DA), Baltimore Orioles (DS *Fellow* — sports fellowship ×4), Cornell (Commercialization DA — tech-transfer niche), Corning (QA DA), Behr/Masco (DA Procurement — niche), Vanguard (College-to-Corporate IT Internship — DA), Magnera (IT DA Intern), ConnectPrep (DA Internship), Capital One (DA Intern Summer 2027 — logged).

### Tier 1 — Hot & strong fit (net-new)
_Explicit entry/new-grad marker + on-target title + in-window + commercial (non-defense, non-FAANG) + strong applied-AI/data fit. Visa inferred (JDs all 403)._

| Company | What they do / size | Role | Direct link | Posted (basis) | Visa posture | Fit note | Source |
|---|---|---|---|---|---|---|---|
| Vagaro | SMB SaaS — booking/business-mgmt for salons/spas/fitness; mid-size private | **Jr.** AI Engineer | https://jobright.ai/jobs/info/6a45bf2bc2d11a6a4666a001 | Jul 09 (jobright cal) | Silent, small-mid — likely OK | **Explicit "Jr." + AI Engineer** at a commercial SaaS co — cleanest net-new entry applied-AI title this run; Pleasanton CA (onsite); shipping LLM features into a real product maps to the ship-real-systems profile; gap: onsite Bay Area, seniority beyond "Jr." unverified (JD 403) | jobright ENG |
| Quora | Q&A knowledge platform / Poe AI-chat app; large private (non-FAANG) | Machine Learning Engineer, **New Grad** (Remote) | https://jobright.ai/jobs/info/6a4f0064fc327f422fef4b01 | Jul 08 (jobright cal) | Silent, larger co — **verify** (Poe = LLM product, likely OPT-OK) | **Explicit "New Grad" MLE + Remote-US** at an LLM-product company (Poe) — title/level/remote all on-target; applied-ML + LLM fit is strong; gap: large-co immigration policy unread, recsys/ranking may lean more ML-systems than RAG | jobright ENG |

### Tier 2 — Worth a look (net-new; caveat = seniority and/or visa unverified, JD 403)

| Company | What they do / size | Role | Direct link | Posted (basis) | Visa posture | Fit note | Source |
|---|---|---|---|---|---|---|---|
| Rakuten International | E-commerce / fintech / digital-content conglomerate (US arm); large | Early Career AI Engineer (**Contract**) | https://jobright.ai/jobs/info/6a4ea9621544d7246c0d3b71 | Jul 08 (jobright cal) | Silent, larger co — **verify** | **Explicit "Early Career" + AI Engineer**; contract is in-scope per brief; Bellevue WA / San Mateo CA; applied-AI fit; gap: contract (not FTE), large-co policy, onsite | jobright ENG |
| Janus Henderson Investors | Global asset-management firm; large public | **Junior** Data Engineer | https://jobright.ai/jobs/info/6a4f975c9469c0662034e31d | Jul 09 (jobright cal) | Silent, larger co — **verify** | **Explicit "Junior" DE**; Denver CO; dbt/SQL/Airflow/pipeline fit; gap: finance-data domain, large-co immigration policy | jobright ENG |
| Axos Bank | Branchless digital bank / fintech; mid-large public | JBATA — Data Engineer (associate-training program) | https://jobright.ai/jobs/info/6a50dd1001740136c2142004 | Jul 10 (jobright cal) | Silent, mid-large — **verify** | "JBATA" reads as a junior/associate rotational bank-analyst track → likely entry rung; San Diego CA; SQL/DE fit; gap: banking-analytics domain, confirm the JBATA program is entry-level and OPT-eligible | jobright ENG |
| Somatus | Value-based kidney-care health-tech; growth-stage private | Machine Learning Engineer (Remote option) | https://jobright.ai/jobs/info/6a4e9fe30ea38951a6ff3ec0 | Jul 07–08 (jobright cal) | Silent, growth-stage — likely OK, **verify** | MLE target title at a growth health-tech co; McLean VA (a remote variant also listed); applied-ML fit; gap: **no entry marker → seniority unverified**, healthcare-data domain | jobright ENG |
| AcuityMD | Medtech commercial-intelligence SaaS; growth-stage startup | Data Analyst, **Software Engineering Track** (Remote) | https://jobright.ai/jobs/info/6a513da5ae4052672fe982b6 | Jul 08–10 (jobright cal) | Silent, growth startup — likely OK | DA on an **eng track + Remote-US** at a funded startup — analytics-engineer-flavored DA maps well to dbt/SQL/analytics-eng profile; Boston HQ but remote; gap: seniority-verify, medtech domain | jobright ENG |
| Generate:Biomedicines | Generative-AI biotech (protein/therapeutic design); large private (well-funded) | Data Scientist **I** | https://jobright.ai/jobs/info/6a4fe35b397d8d353c28d003 | Jul 09 (jobright cal) | Silent, larger private — **verify** | **Explicit "I" entry** DS at a generative-AI-first biotech; Somerville MA (onsite); ML/DS + generative-AI-adjacent domain fit; gap: bio/wet-lab-adjacent data may want domain background, onsite | jobright DA |
| CareFirst BlueCross BlueShield | Regional health insurer (non-profit-ish, mid-Atlantic); large | **Associate** Data Analyst (Hybrid) | https://jobright.ai/jobs/info/6a4e4d259469c0662034ac35 | Jul 08 (jobright cal) | Likely OPT-friendly (large health-insurer, routine OPT hiring), **verify** | **Explicit "Associate" DA**; Baltimore MD hybrid; SQL/BI/reporting fit; gap: healthcare-payer-analytics niche, hybrid onsite | jobright DA |
| Trinity Health | Large national Catholic non-profit health system; very large (cap-exempt-adjacent) | **Associate** Data Scientist (Remote) | https://jobright.ai/jobs/info/6a4eb19ffc327f422fef3d17 | Jul 08 (jobright cal) | Likely OPT-friendly (non-profit health system), **verify** | **Explicit "Associate" DS + Remote**; non-profit health system = lower visa risk; SQL/ML/analytics fit; gap: healthcare domain, DS-not-applied-AI-eng | jobright DA |
| Boston Consulting Group (BCG) | Global management-consulting firm; very large | **Junior** Analyst — BCG Vantage, GenAI / People & Org | https://jobright.ai/jobs/info/6a4ea5619469c0662034c2b8 | Jul 08 (jobright cal) | Silent, larger co — **verify** | **Explicit "Junior" + GenAI** knowledge-analyst role at BCG; Boston; touches the People-Analytics + GenAI target area; gap: consulting-knowledge-team (not product eng), large-co policy | jobright ENG |

### Tier 3 — Long shots / needs verification / seniority, domain, or legitimacy risk (net-new)

| Company | What they do | Role | Direct link | Posted | Visa posture | Why Tier 3 | Source |
|---|---|---|---|---|---|---|---|
| Kikoff | Consumer credit-building fintech; growth-stage startup | Data Scientist, Operations | https://jobright.ai/jobs/info/6a5101ea57513b72e0c62fcf | Jul 10 (jobright cal) | Silent, growth startup — likely OK | On-target DS at a funded fintech startup; SF (onsite); **no entry marker → seniority unverified**; ops-DS may skew analytics-reporting vs applied-AI | jobright DA |
| TransRe | Global reinsurance firm; large | Data Analyst | https://jobright.ai/jobs/info/6a518d0278e364789ca5a33a | Jul 10 (jobright cal) | Silent, larger co — **verify** | DA target; NYC (onsite); SQL/BI fit; **no entry marker → seniority unverified**, reinsurance-analytics niche | jobright DA |
| PhysicsX | AI-for-engineering-simulation (physics ML); growth-stage (UK-HQ, US roles) | Data Scientist | https://jobright.ai/jobs/info/6a3e6a884d047136e0936162 | Jul 07 (jobright cal) | Unclear — **verify** (UK-HQ co, confirm the role is a US-based US-work-authorized req) | Strong applied-ML mission fit (simulation ML); SF listing; **no entry marker → seniority unverified**, UK-HQ → verify US work-authorization path | jobright DA |
| Alta Resource Technologies | Small/opaque data/IT-services vendor; small | **Junior** Data Engineer | https://jobright.ai/jobs/info/6a4e21d89469c0662034a732 | Jul 08 (jobright cal) | Unclear — **verify** | Explicit "Junior" DE but **opaque small vendor → verify legitimacy/scope before effort** (staffing-shop risk pattern); Boulder CO | jobright ENG |
| Burlington Stores | Off-price retail chain; large public | AI Data Scientist | https://jobright.ai/jobs/info/6a2a5ddcc07d4b6ae1c45917 | Jul 07 (jobright cal, ~repost) | Silent, larger co — **verify** | "AI Data Scientist" title fit; Beverly NJ (hybrid); **no entry marker → seniority unverified**, retail-analytics; older req id (possible earlier post reposted) | jobright ENG |
| CarParts.com | E-commerce auto-parts retailer; mid-large public | Data Analyst | https://jobright.ai/jobs/info/6a5003ddfc327f422fef76c2 | Jul 09 (jobright cal) | Silent, mid-large — **verify** | DA target; Long Beach CA (onsite); SQL/BI/e-commerce-analytics fit; **no entry marker → seniority unverified** | jobright DA |
| Sunsweet Growers | Agricultural grower-cooperative (dried fruit); mid-size | Business Intelligence Developer | https://jobright.ai/jobs/info/6a4fcdf01544d7246c0d6812 | Jul 09 (jobright cal) | Silent, mid-size — likely OK, **verify** | **BI Developer target title**; Yuba City CA (rural, onsite); Power BI/SQL/BI fit; gap: rural onsite, seniority-verify, ag-co-op domain | jobright DA |
| Intelliswift (an LTTS Company) | IT-services / digital-engineering staffing firm; large | **Junior** Data Analyst | https://jobright.ai/jobs/info/6a29a3cc1de59e0682a809b2 | Jul 08 (jobright cal, ~repost) | Silent, larger co — **verify** | **TX in-state (Plano)** + explicit "Junior" DA; SQL/BI fit; gap: **IT-services staffing shop → may bench/staff to client accounts** (scope risk), older req id | jobright DA |
| Saratoga County Chamber / Hudson River Community CU | Local chamber of commerce + community credit union (likely same cross-posted req); small | Analytics Engineer **I** | https://jobright.ai/jobs/info/6a50eec08d7d3e6cf1cbd81b | Jul 09–10 (jobright cal) | Silent, small — likely OK, **verify** | **Explicit "I" Analytics Engineer** (rare title match) but **tiny local orgs** (Corinth NY, onsite/hybrid) — small employers sometimes can't accommodate OPT; low comp/scope likely; verify legitimacy | jobright ENG |
| A.P. Moller – Maersk | Global shipping/logistics giant; very large | Data Analyst | https://jobright.ai/jobs/info/6a4ff379397d8d353c28d499 | Jul 09 (jobright cal) | Unclear — **verify** (jobright company link resolved to maerskperu.com → confirm this is the US Cinnaminson-NJ req, not a LATAM posting) | DA target; Cinnaminson NJ (onsite); logistics-analytics fit; gap: verify it's a US-authorized req, large-co policy, seniority-verify | jobright DA |
| Ryan Specialty | Specialty insurance holding company; large public | Data Analyst | https://jobright.ai/jobs/info/6a510cc0bf63b66c79978705 | Jul 10 (jobright cal) | Silent, larger co — **verify** | DA target; Richmond VA (onsite); SQL/BI/insurance-analytics fit; **no entry marker → seniority unverified** | jobright DA |
| Gotion Inc. | EV-battery manufacturer (Chinese-parent, US ops); large | Data Analyst | https://jobright.ai/jobs/info/6a51b48357513b72e0c664ca | Jul 10 (jobright cal) | Unclear — **verify** | DA target, Fremont CA; **CAVEAT: Chinese-parent battery maker with US-gov scrutiny (CFIUS/security-sensitive) → some roles may carry citizenship/security constraints** even though commercial; verify before effort | jobright DA |
| TriWest Healthcare Alliance | Administers healthcare for US military/VA beneficiaries; large | Data Analyst (Remote) | https://jobright.ai/jobs/info/6a51b8fd02522b5b722ea403 | Jul 10 (jobright cal) | Unclear — **verify** | DA + Remote fit, but **CAVEAT: TriWest administers VA/DoD healthcare → some roles carry US-person/background constraints**; Phoenix AZ (remote); verify no citizenship clause | jobright DA |
| University of Rochester (Advancement) | R1 university fundraising/advancement office; large (cap-exempt employer) | Data Analyst | https://jobright.ai/jobs/info/6a5245898576ec69c014b39e | Jul 11 (jobright cal) | Likely OPT-friendly (university, H-1B cap-exempt), **verify** | Cap-exempt employer = lower visa risk; **CAVEAT: this is the SAME "University of Rochester Advancement" (fundraising-ops) team flagged/excluded in a prior run** — donor/fundraising analytics is a niche, non-applied-AI fit; kept in Tier 3 for the cap-exempt visa angle only | jobright DA |

### Community / informal leads (YC Work-at-a-Startup — real direct links from WebSearch; JD pages 403 → seniority/visa/post-date UNVERIFIED)
_All are YC startups → visa: silent, small startup (OPT needs no sponsorship, generally OK; tiny startups sometimes can't accommodate — verify). Open each to confirm entry-level + US-eligibility before applying. Net-new only (FurtherAI, Mercura, Cekura, Paragon, HackerRank, StackAI, Retell, Cedana, Trellis AI, HappyRobot, Bild, Saphira, Kastle, etc. already logged/excluded)._
- **Klarity** — AI Backend Engineer: doc-processing / contract-review AI. https://www.ycombinator.com/companies/klarity/jobs/LcmNldo-ai-backend-engineer — **WebSearch snippet says "1-3 years OR exceptional new grads with strong portfolios"** → explicitly new-grad-open; SF/Bay Area; strong applied-AI + portfolio-driven fit (candidate has a live RAG platform + fine-tuned LLM to show). **Best net-new community lead this run.** Seniority/visa JD-unverified (403).
- **AssemblyAI** — Forward Deployed Engineer: speech-AI / voice-agent API platform (well-funded). https://www.ycombinator.com/companies/assemblyai/jobs/m0Ng8am-forward-deployed-engineer — FDE target at a funded AI-infra co; customer-facing deploy of AI maps to the ship-real-systems + agent profile. Seniority unverified.
- **Eloquent AI** — AI Engineer, Platform: AI-agents for regulated/financial workflows. https://www.ycombinator.com/companies/eloquent-ai/jobs/sJHxcvc-ai-engineer-platform — AI Engineer target + agent/LLM domain overlaps the candidate's LangGraph/RAG-financial-intelligence work. Seniority unverified.
- **Saturn** — AI Engineer: https://www.ycombinator.com/companies/saturn/jobs/0RDVUZG-ai-engineer — AI Engineer target title at a YC startup; **CAVEAT: a "Senior AI Engineer" variant exists at the same company → the plain "AI Engineer" req may still be mid-level; seniority unverified** (403). Tier-3-equivalent community lead.
- **Informal / HN (link NOT captured — thread 403):** **Prolific — AI Solutions Engineer** (applied automation / ML / agentic engineering) surfaced only in an HN-thread WebSearch *snippet*; on-target title (AI Solutions Engineer) and Prolific is a legitimate UK/US research-data company, but **no stable direct apply link could be retrieved** (HN 403 on all paths). Logged honestly without a fabricated URL — verify Prolific's careers page manually for a US-based, US-work-authorized req.

### Intern / Co-op (in-scope per brief, but timing-flagged for a May-2026 grad → OPT starts Jul 13 2026)
_Listed for completeness, NOT recommended over FTE — a Fall/Summer-2026 or 2027 intern/co-op conflicts with a new-grad FTE start and burns OPT runway on a non-FTE rung._
| Company | Role | Direct link | Posted | Visa | Note |
|---|---|---|---|---|---|
| Cotiviti | Intern — AI Engineer (Remote) | https://careers-cotiviti.icims.com/jobs/19531/intern---ai-engineer/job | speedyapply 2d first-seen (~Jul 9) | Silent, larger co — verify | Healthcare-payment-analytics co; **Remote-US AI-eng intern** — the most on-profile intern this run, but intern rung + timing |
| ING | Intern — Data Scientist — GenAI Chatbot (Waco, TX +1) | https://ing.wd3.myworkdayjobs.com/en-US/icsgblcor/job/ (Waco-TX variant) | speedyapply 10d first-seen (~Jul 1) | Silent, larger co — verify | **TX in-state (Waco)** + GenAI; intern rung; note the parent req is Amsterdam-based (non-US) → confirm the US Waco variant |
| Nelnet | Intern — AI Engineer (Lincoln, NE) | https://nelnet.wd1.myworkdayjobs.com/en-US/mynelnet/job/Lincoln-NE/Intern | speedyapply 7d first-seen (~Jul 4) | Silent, larger co — verify | Student-loan-servicing fintech; AI-eng intern; intern rung |

### Recency note
All tiered jobright rows carry calendar post dates **Jul 07 – Jul 11 2026**, comfortably inside the 30-day (Jun 11 – Jul 11) window. speedyapply intern rows carry day-age first-seen stamps (2d–10d → ~Jul 1–9), inside window. **YC community rows and the Prolific/HN informal lead have NO verifiable post date (JD/thread 403) → recency-unverified, flagged inline.** No lead in the main tiers falls outside the window, so no separate "Recency unverified" tier is needed — the JD-403 community/informal rows are the only undatable ones and are marked as such.

### Bonus — adjacent titles & search-vocabulary notes
- **"JBATA" (Axos Bank)** — bank associate-training-analyst program acronyms are a hidden entry-rung signal; worth grepping bank postings for program-name acronyms rather than the word "junior."
- **"Data Analyst, Software Engineering Track" (AcuityMD)** — analytics-engineer-flavored DA titles ("...SWE track", "Analytics Engineer I") remain the highest-signal fit for this profile's dbt/SQL/analytics-eng half; keep parsing for the "I"/"Track" qualifiers.
- **Cap-exempt / non-profit health & education = lower visa risk:** Trinity Health (Assoc DS), CareFirst (Assoc DA), University of Rochester (DA) this run — non-profit health systems, insurers, and universities are the materially lower-risk visa bucket; weight up.
- **Generative-AI-native non-software domains** (Generate:Biomedicines — generative-AI biotech) surface real applied-ML "DS I" roles outside the usual SaaS pool; the candidate's generative/fine-tuning work is a genuine hook there.
- **YC `site:ycombinator.com/companies "Forward Deployed Engineer" / "AI Engineer"` remains the ONLY working community channel** — it refilled 4 net-new this run (Klarity, AssemblyAI, Eloquent AI, Saturn). Run both queries every pass.

### TODO for next (daily) run
- **CONCURRENCY:** 8th backfill on `main` in ~8 days. Always `git fetch origin main` and read the LATEST log before de-duping — one-day-apart passes over the same sources are ~90%+ overlap. Expect a short net-new list; do NOT pad.
- **REPO BRANCHES (hard-code to avoid 404 round-trips):** `jobright-ai/2026-*-New-Grad` repos are on **`master`**; `speedyapply/2026-AI-College-Jobs` + `2026-SWE-College-Jobs` on **`main`**. The standalone `speedyapply/NEW_GRAD_USA` repo **404'd this run** — the best-explicit-"I" data may now live at `raw.githubusercontent.com/speedyapply/2026-AI-College-Jobs/main/NEW_GRAD_USA.md` (a *file*, not a repo); **try that path next run.** `zapplyjobs` repo names not re-confirmed this run (prior runs used it as a workhorse for the H-1B/Sponsor column) — re-locate the correct `zapplyjobs/*` repo next run.
- **STRUCTURAL BLOCKERS (all re-confirmed 403/000 this run, unchanged):** individual JD verification (jobright/greenhouse/ashby/workday/YC) = 403 WebFetch / 000 curl → all visa buckets inferred. HN July thread `48747976` = 403 on news.ycombinator.com AND all three mirrors (hnhiring, nchelluri, dheerajck) → HN still fully un-minable. Reddit/Discord/Slack still hard-blocked. LinkedIn/Indeed/Glassdoor/Wellfound = aggregator pages only.
- **Keep curl + local Python parse of the GitHub tracker READMEs** (all HTTP 200) as the reliable spine; WebSearch YC as the only community supplement.

---

## Run: 2026-07-12 17:15 (DEEP BACKFILL — 30 day window)

**This is a one-time DEEP BACKFILL, not the daily 24h pass** — window is the last **30 days (≈Jun 12 – Jul 12 2026)**. This is the 9th backfill on `main` in ~9 days (07-08 → 07-11 → 07-12), so overlap with prior runs is extreme; the net-new list is deliberately short and NOT padded. Every lead is cross-checked against the full prior log (company + role); exact repeats are dropped. **Highlight this run:** correctly parsing the speedyapply `NEW_GRAD_USA.md` "Other" table (5-col, no salary) surfaced several **direct company-site/ATS links** (UT Austin, Lurie Children's, Modea, SynergenX) — materially better than the 403-walled jobright links, since these open without auth.

### Coverage & honesty notes (read before trusting anything below)
- **Sources covered FULLY (machine-read, HTTP 200 via curl → local Python parse):** `jobright-ai/2026-Engineering-New-Grad` (master, 7.7k lines), `jobright-ai/2026-Data-Analysis-New-Grad` (master), `jobright-ai/2026-Business-Analyst-New-Grad` (master — 0 in-scope net-new), `speedyapply/2026-AI-College-Jobs` (main) + its **`NEW_GRAD_USA.md`** file (main — this run FIXED the prior parser bug: the "Other" new-grad table is 5 columns, was being dropped; 160 rows now parsed, 41 in-window/in-scope/non-FAANG, 6 genuinely net-new). `speedyapply/2026-SWE-College-Jobs` (main — 0 net-new in-scope). YC Work-at-a-Startup via WebSearch (`site:ycombinator.com/companies` AI-Engineer / new-grad queries).
- **Sources NOT covered / BLOCKED this run (structural blockers, re-confirmed live):**
  - **Individual JD verification — STILL 403 on everything.** Re-tested live: `WebFetch` to jobright.ai (dv01 req) = **403**. So EVERY jobright-sourced visa bucket below is inferred from company type + role, NOT JD text. The speedyapply direct ATS links (UT Austin/Lurie/Modea/SynergenX/Andrew Morgan) were NOT individually fetched (time budget) but are stable, auth-free apply URLs — open them directly to read JD + visa.
  - **HN "Ask HN: Who is hiring? (July 2026)" thread `48747976`** — re-tested this run: `WebFetch` to `hnhiring.com/search` = **403**. HN remains machine-unreadable on every path across all runs. Named leads leaked via WebSearch *snippets only* (NO clickable per-post link retrievable): **Kadoa** (lean remote team, software-craft/data/AI — apply by emailing adrian@kadoa, mention HN), **Ojin** (real-time AI voice agents on distributed GPU fabric), **LightSight** (forward-deployed AI consulting, ex-Microsoft). **Cardog** surfaced again = Toronto/non-US → excluded. Logged honestly without fabricated URLs — verify each company's careers page manually.
  - **Reddit / Discord / Slack** — not machine-readable, hard-blocked every prior run; not re-tested (no working path exists).
  - **LinkedIn / Indeed / Glassdoor / Wellfound / Built In** — only aggregator landing pages, no per-posting dates/stable links → nothing logged. No founder/recruiter post with a stable direct link surfaced this run.
- **Recency basis:** jobright rows carry calendar post dates (all Jul 07–Jul 12, inside window). speedyapply rows carry day-age first-seen stamps (3d–18d → ~Jun 24–Jul 9, inside window). YC community rows have **no verifiable post date** (JD 403) → recency-unverified, flagged inline.
- **De-duped OUT as already logged** (surfaced again this run with the SAME company+role, NOT repeated below): Amigo (Applied AI Engineer — Pre-Sale), Oscar Health (Analytics Engineer I), SyntheticFi (Founding Data Analyst), Capgemini (Junior Power BI Developer + Junior DE/ETL), Beth Israel Lahey (Healthcare Data Analyst), Veolia (Meter Data Analyst), OneRail (Junior Data Analyst), Gen/Gen Digital (MLE I), Quora (MLE New Grad — incl. the Jobgether/Ashby repost mirrors), Generate Biomedicines (Data Scientist I — now also has a direct greenhouse link), Frontier Medicines (AI Engineer I), Janus Henderson (Junior DE), Somatus (MLE), Rakuten (Early Career AI Eng), AcuityMD (DA SWE-Track), Innodata (Applied AI Analyst/Associate), Chime, Together AI, CRISP DC, AssetMark, CyberProof/UST/UST HealthProof (Assoc DE — all city variants), Capital One (DA Associate 2027), Quantifind (Assoc DS), Northwell (BI Analyst), AEG (Assoc DS), WaFd Bank (BI Analyst — kept-out prior), CharterUP (DA), Danaher (DA Global Trade), PTC (Jr DA/Analytics Eng), Gravie (DA), Tocaro Blue (DA), Cornell (Commercialization DA), California Correctional (Research DA I), Liberty University (Business DA I), Astrana Health (FDE), Sitreps 2 Steercos (FDE), plus prior YC (Klarity, AssemblyAI, Eloquent AI, Saturn, StackAI, HackerRank, FurtherAI, Mercura, Cekura, Paragon, etc.).
- **Excluded by hard filter this run (logged so future runs don't re-surface):**
  - *FAANG / near-FAANG / referral-channel:* Amazon, TikTok/ByteDance (many MLE/Applied-Scientist Graduate rows), Tesla (AI Eng Optimus), SpaceX (AI Eng Special Programs / ML Surrogate / Starlink DE — ITAR), NVIDIA (NCG research/AI-perf), Figma (DS Core Data — PhD), Rivian, Boeing (BI Analyst), CVS Health, PepsiCo, XPENG, Point72/Jump Trading (prop-quant), BlackRock (Modelling DS Associate — near-tier).
  - *Defense / federal / FFRDC / clearance / gov (US-person near-certain):* Booz Allen (DS Junior ×2 San Diego/Springfield), Peraton (Jr Software DE Fort Meade), AMERICAN SYSTEMS, Northrop Grumman (Assoc DS), MANTECH (Jr DS), GDIT (DS Junior Jacksonville NC), KBR (DS I Arlington), A-TEK (Federal AI Solutions Eng Entry + Health IT DA I — federal), Technomics (Cyber Risk DE), Clarity Innovations, General Atomics (DS I), Command Holdings (DA — *Cleared*), Constellis (DA), Elbit Systems, DCCA (MLE), The Nuclear Company (AI Engineer 1), Radiance Technologies (Jr DA), ITA-International (Jr DA), DeVine Consulting (Atmospheric Research Assoc), **The Aerospace Corporation (2026 Data Scientist — Colorado Springs; FFRDC, clearance-likely → excluded)**, **Anduril (Jr People Data Analyst — kept in Tier 3 with a US-person caveat, not fully excluded, since People/HR analytics is less ITAR-bound — verify)**.
  - *Trading-floor / prop-quant / finance-residency:* Jump Trading, Point72, Aptura (multiple "Investment/Leveraged/Corporate Finance Associate — AI Residency" NYC — finance-desk AI residency, off-profile).
  - *Non-US geography (F-1 OPT is US-only):* Bending Spoons (Graduate DS — UK), Baltic Apprenticeships (DA Apprentice — UK), Flight Centre (Jr DE — Toronto), TrendAI (Applied AI Jr Web Dev — Ottawa), Applied Systems Canada, OLG (DS Student — Canada), Connor Clark & Lunn (Quant Equity DA — CA/Canada), Publicis Groupe ANZ (Jr DA — Australia), PepsiCo Canada (Jr DS), Cardog (Toronto).
  - *Co-op / intern / temp / student / fellow / clinical-research / niche-QA (weak timing or non-fit):* SharkNinja (SharkByte Applied AI & Analytics Co-op Fall-2026 ×2), Michael Baker (Pavement DA — *Temp*), U Penn (AI Solutions Specialist — *Temporary*), Baltimore Orioles (DS *Fellow*), Ensign College (Student DA), Northwestern (Research DA Associate ×2 — clinical/infectious-disease), Mass General Brigham (Research DA per-diem; Clinical Research Coord/DA), Cornell (Commercialization DA — tech-transfer niche), Corning (QA DA), Behr/Masco (DA Procurement — niche), Vadilal (kept Tier 3 w/ stale-repost flag), Scopely/Jerry/Billtrust/Berkley/Upgrade/Handshake/Esri/Flashpoint/Careerswift/SBT Global/Hays/Cushman & Wakefield (all previously logged Assoc/Jr rows).

### Tier 1 — Hot & strong fit (net-new)
_Explicit entry/new-grad marker + on-target title + in-window + commercial (non-defense, non-FAANG) + strong applied-AI/analytics-eng fit. Visa inferred where JD is 403._

| Company | What they do / size | Role | Direct link | Posted (basis) | Visa posture | Fit note | Source |
|---|---|---|---|---|---|---|---|
| dv01 | Loan-level structured-finance / consumer-credit data-analytics platform; growth-stage fintech | **Data Analytics Engineer I** (Remote) | https://jobright.ai/jobs/info/6a531d2dd007ee02d95fa58e | Jul 11 (jobright cal) | Silent, growth-stage → likely OK, verify | **Explicit "I" Analytics Engineer + Remote-US at a fintech-data company** — bullseye for the dbt/SQL/analytics-eng half of the profile; loan/credit data is exactly the "financial-intelligence" domain the candidate's flagship RAG platform sits in; gap: JD 403 (visa unread), seniority beyond "I" unverified | jobright ENG |
| University of Texas at Austin | Flagship public research university; very large (H-1B **cap-exempt**) | **Data Engineer I** | https://utaustin.wd1.myworkdayjobs.com/en-US/utstaff/job/AUSTIN-TX/Data-Engineer-I_R_00045134 | speedyapply 18d first-seen (~Jun 24) | **Likely OPT-friendly (public university → H-1B cap-exempt = lowest visa risk)** | **Explicit "I" DE + TX in-state (Austin) + cap-exempt employer** — the lowest-visa-risk lead of the run, DIRECT auth-free Workday link, and geographically ideal (candidate is Richardson TX); SQL/pipeline/DE fit; gap: public-sector pay band, onsite Austin | speedyapply NEW_GRAD |

### Tier 2 — Worth a look (net-new; caveat = seniority and/or visa unverified)

| Company | What they do / size | Role | Direct link | Posted (basis) | Visa posture | Fit note | Source |
|---|---|---|---|---|---|---|---|
| Ann & Robert H. Lurie Children's Hospital of Chicago | Pediatric non-profit hospital / academic medical center; very large | **Data Engineer I** | https://luriechildrens.wd1.myworkdayjobs.com/en-US/externalportal/job/Streeterville-Chicago-IL/Data-Engineer-I_JR2026-1951 | speedyapply 5d first-seen (~Jul 7) | Likely OPT-friendly (non-profit hospital, cap-exempt-adjacent), verify | **Explicit "I" DE + non-profit health system (lower visa risk) + DIRECT auth-free Workday link**; SQL/pipeline/DE fit; gap: healthcare-data domain, onsite Chicago | speedyapply NEW_GRAD |
| Care Access | Decentralized / site-agnostic clinical-trials & research network; growth-stage health-tech | Enterprise Performance **Analytics Engineer** (Remote) | https://jobright.ai/jobs/info/6a53397dd007ee02d95fab63 | Jul 11 (jobright cal) | Silent, growth-stage → likely OK, verify | Analytics-Engineer title + Remote-US at a funded health-tech; dbt/SQL/analytics-eng fit; gap: **no explicit entry marker (seniority unverified)**, "Enterprise Performance" may skew FP&A/BI-reporting over product data, JD 403 | jobright ENG |
| Modea | Digital-experience / product agency (healthcare & enterprise clients); small-mid private | **Data Analyst I** | http://careers.modea.com/apply/IHUDVirHJ7/Data-Analyst-I | speedyapply 3d first-seen (~Jul 9) | Silent, small-mid → likely OK | **Explicit "I" DA + DIRECT auth-free apply link**; SQL/BI/analytics fit at a small agency (OPT often workable at this size); gap: onsite Roanoke VA, agency-client-analytics (breadth over depth) | speedyapply NEW_GRAD |
| SynergenX Health | Men's-health / hormone-therapy clinic network (TX-based); mid-size private | **Junior Data Engineer** | http://synergenx.applytojob.com/apply/iHsR2jKzIR/Junior-Data-Engineer | speedyapply 3d first-seen (~Jul 9) | Silent, small-mid → likely OK | **Explicit "Junior" DE + TX in-state (Houston) + DIRECT auth-free apply link**; SQL/pipeline/DE fit; gap: onsite Houston, healthcare-ops-data domain | speedyapply NEW_GRAD |
| Paramount | Media / streaming conglomerate; large public (non-FAANG) | **Associate** Data Scientist | https://jobright.ai/jobs/info/6a4d663f35e45603c4bb3cb8 | Jul 07 (jobright cal) | Silent, larger co → verify | **Explicit "Associate" DS — NET-NEW role** (prior log had Paramount "MLE, Entry" NY; this is a different req/level/site); ML/DS + media-analytics fit; gap: onsite Burbank, large-co immigration policy unread | jobright DA |
| Mariana Minerals | Minerals-exploration / mining-tech startup (AI for subsurface/geology); early-stage | Machine Learning Engineer | https://jobright.ai/jobs/info/6a2a30391de59e0682a83467 | Jul 11 (jobright cal, ~repost — older req id) | Silent, startup → likely OK, verify | MLE target title at an applied-AI startup; end-to-end ML/shipping fit; gap: **no entry marker (seniority unverified)**, onsite Ann Arbor MI, geoscience domain, older req id (possible earlier post reposted) | jobright ENG |
| Brown University Health (Lifespan) | Non-profit academic health system (RI); very large | Data Analyst | https://jobright.ai/jobs/info/6a500cbe9469c0662034fffb | Jul 09 (jobright cal) | Likely OPT-friendly (non-profit health system), verify | Non-profit health system = lower visa risk; SQL/BI/analytics fit; gap: **no entry marker (seniority unverified)**, healthcare domain, onsite Providence RI, JD 403 | jobright DA |
| Wilson Elser | National full-service law firm (insurance-defense heavy); large | Data Analyst (Hybrid — White Plains / NYC / Stamford) | https://jobright.ai/jobs/info/6a5345158ef95364ead919d6 | Jul 11–12 (jobright cal) | Silent, larger co → verify | DA target at a large law firm; SQL/BI/reporting fit; gap: **no entry marker (seniority unverified)**, legal/e-billing-analytics niche, hybrid onsite | jobright DA |
| Mars | Global CPG (confectionery / pet-care / food); very large private | Site Industrial Data Engineer | https://jobright.ai/jobs/info/6a51b58f8d7d3e6cf1cc1784 | Jul 11 (jobright cal) | Silent, larger co → verify | DE at a very large CPG; pipeline/data fit; gap: **no entry marker (seniority unverified)**, rural onsite (North Sioux City SD), OT/manufacturing-data skew over applied-AI | jobright ENG |

### Tier 3 — Long shots / needs verification / seniority, domain, or legitimacy risk (net-new)

| Company | What they do | Role | Direct link | Posted | Visa posture | Why Tier 3 | Source |
|---|---|---|---|---|---|---|---|
| Andrew Morgan Consulting | Data/analytics consulting & staffing firm; small-mid | Data Engineer I (Remote) | https://andrew-morgan.com/current-openings/?gh_jid=5274924008 | speedyapply 18d first-seen (~Jun 24) | Unclear → verify | Explicit "I" DE + Remote + DIRECT link, but **consulting/staffing firm → bench/client-staffing scope risk** (verify it's a direct FTE, not a client placement) | speedyapply NEW_GRAD |
| CCS Global Tech | IT-services / staffing & digital-solutions vendor; mid | **Entry Level** SQL/BI Developer/QA/DA | https://jobright.ai/jobs/info/6a51d181bf63b66c7997d5ab | Jul 10 (jobright cal) | Unclear → verify | Explicit "Entry Level" BI/DA + on-target stack, but **IT-services staffing-shop pattern (bench/client-staffing risk)**; San Diego onsite; verify scope/legitimacy before effort | jobright ENG |
| SEI (Sunnyvale) | Energy / sustainability data (Decarbonization); mid — **entity ambiguous** | Decarbonization Data Analyst | https://jobright.ai/jobs/info/6a51dfc702522b5b722ecd64 | Jul 10 (jobright cal) | Unclear → verify entity | DA + sustainability domain fit, **but "SEI" is an ambiguous acronym — confirm this is the commercial Sunnyvale energy co, NOT the Software Engineering Institute (FFRDC, clearance)**; onsite/hybrid Sunnyvale CA | jobright DA |
| Anduril Industries | Defense-tech (autonomous systems); large well-funded | **Jr** People Data Analyst | https://jobright.ai/jobs/info/6a4c26ea6189f64e437f1508 | Jul 09 (jobright cal) | Unclear/conflicting → verify | Explicit "Jr" + People-Analytics (a target sub-area), and HR-analytics is less ITAR-bound than product roles — **BUT Anduril is defense-tech and often requires US-person status even for support functions**; verify no citizenship clause before effort; onsite Boston | jobright DA |
| Daily Management Resorts | Hospitality / timeshare-resort operator; mid | Data Analyst (Remote) | https://jobright.ai/jobs/info/6a531664e726ec56126a59c4 | Jul 11 (jobright cal) | Silent → verify | DA + Remote fit; **no entry marker (seniority unverified)**, hospitality/timeshare-analytics niche, small-employer OPT-accommodation risk | jobright DA |
| Brevium | Healthcare-tech (patient-reactivation software for clinics); small-mid | Data Analyst **Assistant** | https://jobright.ai/jobs/info/6a501b6f9469c06620350287 | Jul 09 (jobright cal) | Silent, small → likely OK, verify | "Assistant"-level DA = **sub-entry rung, likely low scope/comp**; SQL/BI fit; onsite American Fork UT; kept for completeness only | jobright DA |
| Scalence L.L.C. | Small/opaque data/analytics vendor; small | **Jr.** Data Scientist | https://jobright.ai/jobs/info/6a4eaf720ea38951a6ff41f5 | Jul 08 (jobright cal) | Unclear → verify | Explicit "Jr." DS but **opaque small vendor → verify legitimacy/scope** (staffing-shop risk pattern); onsite Westbrook ME | jobright DA |
| Vadilal Industries USA | Ice-cream / packaged-food manufacturer (Indian-parent, US arm); small | Data Analyst | https://jobright.ai/jobs/info/69d987d7f748bc5d08a4bc75 | Jul 10 (jobright cal — **old req id → likely stale repost**) | Silent, small → verify | DA target but **very old req id (69d9…) suggests a stale/recycled posting**; small food co; onsite Bristol PA; verify the req is actually open | jobright DA |
| Influence Mobile | Mobile-gaming / ad-tech (rewarded-play apps); small-mid | QA Engineer — AI & Automation Data Analyst | https://jobright.ai/jobs/info/6a5011d6f57ed168c3417ea0 | Jul 09 (jobright cal) | Silent → verify | Hybrid QA/DA title + Remote; **QA-automation-flavored, not core applied-AI**; SQL/analytics-adjacent; kept for the "AI & Automation" angle only | jobright ENG |

### Community / informal leads (YC Work-at-a-Startup — real direct links from WebSearch; JD pages 403 → seniority/visa/post-date UNVERIFIED)
_All YC startups → visa: silent, small startup (OPT needs no sponsorship, generally OK; tiny startups sometimes can't accommodate — verify). Open each to confirm entry-level + US-eligibility before applying. Net-new only._
- **DeepAware AI (YC S25)** — AI/ML Engineer: secure/autonomous **data-center infrastructure-management (DCIM)** platform. https://www.ycombinator.com/companies/deepaware-ai/jobs/2x0wzJp-ai-ml-engineer (also https://www.workatastartup.com/jobs/79913) — builds RL models for GPU-workload placement/power optimization **+ anomaly-detection pipelines** and works alongside data engineers; **anomaly-detection + production ML maps directly to the candidate's project work**. WebSearch shows no explicit years/seniority gate (startup ownership emphasis) → plausibly new-grad-open. **Best net-new community lead this run.** Seniority/visa JD-unverified (403).
- **Peppr AI (YC)** — AI Developer / AI Engineer: founding AI eng building ML/LLM pipelines + agents into production. https://www.ycombinator.com/companies/peppr-ai/jobs/ZJhISZP-ai-developer-ai-engineer — on-target title + agent/LLM domain overlaps the candidate's LangGraph/RAG work, **BUT WebSearch snippet says "3+ years" → likely NOT entry-level**; logged as a Tier-3-equivalent stretch, verify the actual bar.
- **Informal / HN (links NOT captured — thread + aggregators all 403):** **Kadoa** (lean remote team; software-craft/data/AI; apply by emailing adrian@kadoa, mention HN), **Ojin** (real-time AI voice agents on distributed GPU fabric), **LightSight** (forward-deployed AI consulting, ex-Microsoft). On-profile (AI/ML, remote, FDE-adjacent) but **no stable per-post apply link retrievable** (HN 403 on news.ycombinator.com + hnhiring). Verify each company's careers page manually.

### Intern / Co-op (in-scope per brief, but timing-flagged for a May-2026 grad → OPT starts Jul 13 2026)
_Listed for completeness, NOT recommended over FTE — a 2026 intern/co-op conflicts with a new-grad FTE start and burns OPT runway on a non-FTE rung._
| Company | Role | Direct link | Posted | Visa | Note |
|---|---|---|---|---|---|
| Bloom (YC) | AI Engineering Student Internship (Winter or Summer 2026) | https://www.ycombinator.com/companies/bloom-4/jobs/VAkpMHI-ai-engineering-student-internship-winter-or-summer-2026 | YC (no datable stamp — JD 403) | Silent, small startup — verify | YC founding-team AI-eng internship **with stated potential to convert to full-time on graduation** — the most on-profile intern this run, but intern rung + timing conflict |

### Recency note
All tiered jobright rows carry calendar post dates **Jul 07 – Jul 12 2026**, inside the 30-day (Jun 12 – Jul 12) window. speedyapply rows carry day-age first-seen stamps (3d–18d → ~Jun 24 – Jul 9), inside window. **YC community rows (DeepAware, Peppr, Bloom) and the HN informal leads (Kadoa/Ojin/LightSight) have NO verifiable post date (JD/thread 403) → recency-unverified, flagged inline.** No lead in the main tiers falls outside the window → no separate "Recency unverified" tier needed; the JD-403 community/informal rows are the only undatable ones and are marked as such.

### Bonus — adjacent titles & search-vocabulary notes
- **PARSER FIX (important for future runs):** `speedyapply/2026-AI-College-Jobs/NEW_GRAD_USA.md` (fetch via `raw.githubusercontent.com/speedyapply/2026-AI-College-Jobs/main/NEW_GRAD_USA.md`) has THREE tables — FAANG+ / Quant (6 cols, w/ Salary) and **"Other" (5 cols, NO Salary column)**. A 6-column assumption silently drops the entire "Other" new-grad table, which is where the best non-FAANG "DE I / DA I / MLE I" rows with **direct ATS links** live. Parse "Other" as 5 columns: `Company | Position | Location | Posting | Age`.
- **DIRECT ATS links beat jobright links:** jobright JD pages are 100% 403-walled (visa unreadable), but the speedyapply "Other" table links straight to company Workday/Greenhouse/ApplyToJob pages that open without auth — prioritize speedyapply-sourced rows when a lead exists on both.
- **"Analytics Engineer I" / "Data Engineer I" / "Data Analyst I" (the explicit-"I" qualifier)** remains the single highest-signal fit for this profile's dbt/SQL/analytics-eng half — this run's Tier-1/2 (dv01, UT Austin, Lurie, Modea) are all "I"-tagged. Keep grepping for the trailing "I".
- **Cap-exempt / non-profit health & education = lowest visa risk, weight up:** UT Austin (DE I), Lurie Children's (DE I), Brown University Health (DA) this run — public universities and non-profit hospitals are the materially lower-risk visa bucket AND UT Austin is TX in-state.
- **Adjacent title spotted:** "Enterprise Performance Analytics Engineer" (Care Access) — an FP&A/finance-flavored analytics-engineer variant; and "AI Residency — Finance Associate" (Aptura, NYC, excluded) — a finance-desk AI-residency pattern worth recognizing as off-profile (trading/IB desk, not product eng).

### TODO for next (daily) run
- **CONCURRENCY:** 9th backfill on `main` in ~9 days. Always `git fetch origin main` and read the LATEST log before de-duping — one-day-apart passes over the same sources are ~90%+ overlap. Expect a short net-new list; do NOT pad.
- **REPO BRANCHES (hard-code):** `jobright-ai/2026-*-New-Grad` on **`master`**; `speedyapply/2026-AI-College-Jobs` + `2026-SWE-College-Jobs` on **`main`**. **The best explicit-"I" + direct-link data is in `speedyapply/2026-AI-College-Jobs/main/NEW_GRAD_USA.md` "Other" table — parse it as 5 columns (see PARSER FIX above).**
- **STRUCTURAL BLOCKERS (all re-confirmed 403 this run):** individual JD verification (jobright/greenhouse/ashby/workday/YC) = 403 WebFetch → all jobright visa buckets inferred. HN July thread `48747976` = 403 on news.ycombinator.com AND hnhiring → HN still fully un-minable. Reddit/Discord/Slack still hard-blocked. LinkedIn/Indeed/Glassdoor/Wellfound = aggregator pages only.
- **Keep curl + local Python parse of the GitHub tracker READMEs** (all HTTP 200) as the reliable spine; WebSearch YC as the only working community supplement.
