...
## Run: 2026-06-21 17:09 (DEEP BACKFILL — 30 day window)

**THIS IS A ONE-TIME DEEP BACKFILL, NOT A DAILY PASS.** Third backfill in the
series, wider budget. Recency window = **30 days (~May 22 – Jun 21, 2026)** vs the
normal 24h. Future daily runs should keep 24h and treat everything below as
already-seen. Read `leads_log.md` in full first; de-duped against ALL four prior
runs (06-20 12:00, 06-20 05:21 backfill, 06-20 18:30, 06-20 18:48 backfill).

**Coverage & honesty notes (read before trusting anything below):**
- **Fully covered (loaded via `raw.githubusercontent.com`, not blocked):**
  - `speedyapply/2026-AI-College-Jobs` → `NEW_GRAD_USA.md` — **read in full** (261 rows; ages now run Jun-21-relative, so "5d"=Jun 16, "12d"=Jun 9, "30d"=May 22 = window edge).
  - `jobright-ai/2026-Data-Analysis-New-Grad` — **read in full**, but it still **caps at "last 7 days"** (Jun 15–21 only), so its 30-day depth was unavailable again.
  - `zapplyjobs/New-Grad-Data-Science-Positions` — AI/ML-Engineer + Data-Engineer subsets pulled (richest source; large file, top ~rows by recency captured).
- **Partial / blocked (be explicit):**
  - `SimplifyJobs/New-Grad-Positions` "Data Science, AI & ML" (37 roles) — **still truncates** before that table loads (4th run in a row). Tried the `dev`-branch raw too. Not usable.
- **NOT covered / blocked this run (UNCHANGED structural blockers — I re-tested each):**
  - **Individual JD verification — ALL still HTTP 403:** greenhouse (`job-boards.greenhouse.io/newsbreak/...` tested), `*.myworkdayjobs.com`, `jobs.ashbyhq.com`, `careers.ti.com` / Oracle Cloud, `jobs.smartrecruiters.com`. **I could not read a single JD body**, so EVERY visa bucket below is inferred from company type + role, NOT JD text, and every "verify"/"seniority unverified" flag stands. Same as all 4 prior runs.
  - **HN "Who is hiring (June 2026)" thread `48357725`** — tested **4 paths, all 403:** HN Algolia items API, `hnhiring.com/june-2026`, the `nchelluri.github.io/hnjobs/` GitHub-pages mirror, and `startup.jobs`. WebSearch over hnhiring returned only vague summaries (Trace, Rerun, Laminar, "Founding AI Infra Engineer") with **no per-post direct links** → nothing from HN logged as a real lead this run.
  - **Reddit** — `www.reddit.com/.../search.json` **hard-blocked** ("unable to fetch") again. No Discord/Slack machine-readable.
  - **LinkedIn / Indeed / Glassdoor / Wellfound / ZipRecruiter / startup.jobs** — only aggregator landing pages, **no per-posting dates or stable direct links** → nothing logged from them. A DFW-local search (Dallas/Richardson/Plano) surfaced only these aggregators.
- **Net new value this run:** the trackers have rotated since the 06-20 backfills, surfacing a handful of **genuinely new** in-window rows not in any prior entry (Citi *Junior* AI Developer, BofA *Data Scientist I – Fraud*, Zayo AI Engineer, CVS *Associate* Data Engineer, GumGum DS I, Adswerve, P&G, Enbridge, GlobalFoundries NCG, DebtBook, etc.). Logged below. The bulk of the trackers re-listed already-logged companies (Jerry, KeyBank, PNC, Franklin Templeton, BlackRock quant, Capital One, BMS, Unity, Upgrade, Node, Hitachi, USAA, Celonis, TreeHouse, Smithfield, Marvell, TIFIN, Quadric, Tanium, NewsBreak, State Street, Citi GenAI-Officer, Docker, Cardinal Health, Caterpillar DE, SS&C, Cape, AbbVie, CVS DE-Irving, Elevance, Morningstar, DraftKings, Westinghouse, Quantifind, SyntheticFi, etc.) — **not repeated here.**

### Tier 1 — Hot & strong fit
_None clear the Tier-1 bar this run. The freshest genuinely-new in-window rows are entry analyst/DS/data-eng roles at large enterprises (good target-title fits) but all JD bodies 403'd → visa + seniority unverified, and none is a JD-verified strong applied-GenAI-engineering match. Kept honestly in Tier 2. (The strongest applied-AI titles that rotated up — NewsBreak Applied AI Eng, State Street/Citi GenAI Officer, BlackRock Applied AI — were already logged in the 06-20 18:48 backfill.)_

### Tier 2 — Worth a look (good fit; caveat = visa and/or seniority unverified, JD 403)

| Company | What they do / size | Role | Direct link | Posted (basis) | Visa posture | Fit note | Source |
|---|---|---|---|---|---|---|---|
| Citi | Global bank; large public | Junior Data Services and AI Developer – Officer | https://citi.wd5.myworkdayjobs.com/en-US/2/job/Jersey-City-New-Jersey-United-States/Junior-Data-Services-and-AI-Developer---Officer_26967855 | "16d" (≈Jun 5) | Silent, larger company — **verify** | **Explicit "Junior" + AI Developer** (distinct from the already-logged Citi GenAI-Officer in Jacksonville); Python/data+AI fit; gap: large-bank immigration policy | speedyapply NEW_GRAD |
| Bank of America | Global bank; large public | Data Scientist I – Fraud Model Governance | https://ghr.wd1.myworkdayjobs.com/en-US/us-emplsv/job/Charlotte/Data-Scientist-I----Fraud-Model-Governance_26019387 | "11d" (≈Jun 10) | Silent, larger company — **verify** | **"I" = entry**; fraud-model DS, SQL/ML/analytics fit (distinct from already-logged BofA Client Quant Analyst I); gap: large-bank policy + GenAI angle | speedyapply NEW_GRAD |
| Zayo | Fiber/network infrastructure (bandwidth/connectivity); large private | AI Engineer | https://zayo.wd1.myworkdayjobs.com/Zayo_Careers/job/United-States/AI-Engineer_R0016139 | "2d" (≈Jun 19) | Silent, larger company — **verify** | **AI Engineer target title + remote-US**; applied-AI fit; gap: **seniority unverified** (JD 403, not labeled entry) | zapplyjobs DS |
| CVS Health | Health services/pharmacy; large public | Associate Data Engineer | https://cvshealth.wd1.myworkdayjobs.com/CVS_Health_Careers/job/RI---Woonsocket/Associate-Data-Engineer_R0942229 | "13m" first-seen (≈Jun 21) | Silent, larger company — **verify** | **"Associate" = entry** Data Engineer (distinct from the already-logged CVS DE-Irving + Manager reqs); dbt/SQL/Airflow fit; gap: large-co policy | zapplyjobs DS |
| GumGum | Contextual-AI advertising (computer vision/NLP on content); mid-size | Data Scientist I | https://job-boards.greenhouse.io/gumgum/jobs/7703520003 | Jun 21 (jobright; reposted — speedyapply also lists, older "age") | Silent, mid-size — verify | **DS I** at an AI-ad company; CV/NLP + analytics fit; mid-size lowers visa risk a notch; gap: ad-domain | jobright DA + speedyapply |
| Adswerve | Google/marketing-data partner & analytics consultancy; small-mid | Data Scientist 1 | https://jobright.ai/jobs/info/6a3024c5524fd5657fe0f12d | Jun 15 | Silent, small-mid — likely OK | **Remote-US, entry "Data Scientist 1"**; analytics/GCP/BI fit (candidate has GCP); smaller shop lowers visa risk; gap: marketing-analytics not applied-GenAI | jobright DA |
| Procter & Gamble | Consumer goods; large public (non-FAANG) | Data Scientist | https://jobright.ai/jobs/info/68cc1b0b128dc347fd91f322 | Jun 19 (jobright) | Silent, larger company — **verify** | DS at scale, Cincinnati; analytics/ML fit; gap: **seniority unverified** + CPG domain + large-co policy | jobright DA |

### Tier 3 — Long shots / needs verification / seniority or domain risk

| Company | What they do | Role | Direct link | Posted | Visa posture | Why Tier 3 | Source |
|---|---|---|---|---|---|---|---|
| GlobalFoundries | Semiconductor foundry; large public | AI Enablement Strategist – New College Graduate | https://globalfoundries.wd1.myworkdayjobs.com/en-US/external/job/USA---Texas---Austin/AI-Enablement-Strategist--New-College-Graduate-_JR-2600367 (also Malta NY: JR-2600365) | "18d" (≈Jun 3) | Silent, larger company — **verify** | **Austin, TX option + explicit NCG**; adjacent to AI Solutions/FDE, BUT "enablement strategist" reads more adoption/ops than build; semis foundry may have some export-control nuance | speedyapply NEW_GRAD |
| Enbridge | Energy/pipeline & utility; large public | Data Analyst Specialist I – Work Management Technology | https://enbridge.wd3.myworkdayjobs.com/en-US/enbridge_careers/job/Houston-TX-USA/Data-Analyst-Specialist-I--Work-Management-Technology_71806 | "8d" (≈Jun 13) | Silent, larger company — **verify** | **Houston, TX** + Data Analyst I target title; SQL/BI fit; gap: ops/asset-mgmt analytics not AI-eng; energy-infra co policy unread | speedyapply NEW_GRAD |
| DebtBook | Muni/public-finance fintech SaaS; growth-stage | Associate Quantitative Strategist | https://job-boards.greenhouse.io/debtbook/jobs/4698960005 | "25d" (≈May 27) | Silent, growth startup — likely OK | Entry "Associate" quant at a fintech; quant-analyst-adjacent (non-trading-floor); Python/SQL fit; gap: finance-domain, seniority of "Strategist" unverified | speedyapply NEW_GRAD |
| Cross River Bank | Banking-as-a-service fintech; mid-large | Associate/AVP – Quantitative Strategies Group | https://www.crossriver.com/greenhouse?gh_jid=7770430003 | "9d" (≈Jun 12) | Silent, larger company — **verify** | Quant-strategies fit at a fintech bank; gap: title spans Associate→AVP so **seniority unverified**; bank policy unread | speedyapply NEW_GRAD |
| Blackstone | Private equity/alt-asset manager; large public | Data Engineer Analyst (Technology & Innovations) | https://blackstone.wd1.myworkdayjobs.com/bx_external_site/job/New-York/XMLNAME-2024-Blackstone-Technology-and-Innovations---Data-Engineer-Analyst_29118 | "1h" first-seen (≈Jun 21) | Silent, larger company — **verify** | **"Analyst" = entry** DE at a PE firm; SQL/pipeline fit; gap: finance-domain + NYC onsite + large-co policy | zapplyjobs DS |
| RBC | Global bank (Royal Bank of Canada, US arm); large public | Quants Associate – Data Science | https://jobright.ai/jobs/info/6a2e45837cd40a338faea2c0 | Jun 17 (jobright) | Silent, larger company — **verify** | Entry "Associate" quant-DS, NY; analytics/ML fit; gap: bank policy + quant-finance domain | jobright DA |
| WorldQuant | Quantitative investment-management fund; mid-large | Junior Quantitative Analyst | https://job-boards.greenhouse.io/worldquant/jobs/4616499006 | "12d" (≈Jun 9) | Silent, larger company — **verify** | **Austin, TX** + explicit "Junior" Quant Analyst target title; Python/SQL fit; gap: this is a **trading/alpha fund** (the "non-trading-floor" caveat on the target is borderline here) | speedyapply NEW_GRAD + jobright |
| Carnegie Mellon University | University research lab (regular CMU, NOT the SEI/FFRDC) | Associate Machine Learning Engineer – Secure AI Lab | https://cmu.wd5.myworkdayjobs.com/en-US/cmu/job/Pittsburgh-PA/Associate-Machine-Learning-Engineer---Secure-AI-Lab_2024609 | "12d" (≈Jun 9) | Unclear — **verify** | Entry "Associate ML Engineer", real applied-ML; gap: "Secure AI" lab may carry funding/citizenship constraints; academic pay/structure | speedyapply NEW_GRAD |

### Adjacent "AI Operations / Implementation / Solutions Associate" roles (note, not full rows)
_A whole cluster of **entry "AI ___ Associate"** titles is live this cycle — mostly **customer/internal AI adoption & ops, NOT engineering builds**, so they're flagged for awareness rather than tiered. Screen the JD; many are GTM/ops-leaning._
- **JustAnswer – Associate AI Conversation Designer** (US remote, "5d"/"9d") — https://job-boards.greenhouse.io/justanswer/jobs/8509749002 — conversation/prompt design, adjacent to applied-AI.
- **Nevoya – AI Operations Associate** (Austin TX & SF, "9d") — https://jobs.ashbyhq.com/nevoya/3163e7df-5c59-4b8a-a37a-15caf2e543d5 — **Austin TX** option; ops not eng.
- **Loop – AI Operations Associate** ("12d") — https://job-boards.greenhouse.io/loop/jobs/5819779004 — ops.
- **Solace – People Operations & AI Associate** (US, "8d") — https://jobs.ashbyhq.com/solace/10e59c85-4919-46b7-85bc-cb7c59657584 — **maps to the People/HR-Analytics target title**, AI-flavored; worth a look for that vertical.

### Notable but OUTSIDE the 30-day window (candidate-local / exact-title — flagged for awareness, NOT counted as a fresh lead)
- **Texas Instruments – New College Graduate, AI Solutions Engineer** — **Richardson/Dallas, TX (candidate-local!)** — direct link **https://careers.ti.com/en/sites/CX/job/25008812** — **posted 04/24/2026 (~58d, outside window)** per careers.ti.com. Exact target title ("AI Solutions Engineer") + NCG + local + JD describes RAG, LLM agents, data pipelines, drift monitoring = **bullseye on the profile**. May well still be open — recommend the candidate check/apply directly even though it predates the 30-day window. Visa: large semis co → verify (careers.ti.com/Oracle 403'd, JD unread).
- **Texas Instruments – Data Engineer, Smart Manufacturing & Automation** — Richardson, TX — posted ~04/21/2026 (outside window); semiconductor-mfg data modeling. Same "local + worth checking directly" note. (No clean per-req direct link captured; found via careers.ti.com search, which 403s the fetcher.)

### Excluded (logged so future runs don't re-surface)
- **FAANG-tier (referral track):** Google (Data Engineer US), Microsoft (Data Engineer / II), Amazon/AWS (SDE ML Networking, DE PV Prime Video, DE II IAM & Abuse, DE WW Ops Finance, DE Decision Intelligence, DS I SCOT-Inbound), TikTok (many ML/Applied-Scientist grad roles).
- **Defense / federal / clearance (US-person near-certain):** Booz Allen (Cybersecurity AI/ML McLean, AI/ML Arlington, DE McLean, Special Missions DE Fort Benning, Azure DE McLean), GDIT (Agentic AI Eng Falls Church, Human Performance DS I), Leidos (SME AI/ML, Computer Vision AI/ML, CMDB DE), KBR (TAGM DE), Amentum (DE Mid Pearl Harbor), Anduril (DE Quality Intelligence; Jr People DA), Defense Unicorns (FDE DE Space — already logged), Raytheon (Algorithm Systems Eng II Plano), KBR/Integral/AITHERAS/Lentech/Everwatch/Bluehawk/Dynamis/Steampunk/Radiance (junior DS/DA roles), Guidehouse & Palantir (Deployment Strategist – Intel), Institute for Defense Analyses, GET (Declassification Analyst).
- **Non-US geography (F-1 OPT is US-only):** Traction Complete & AlgaeCal & Amexio (Canada), Boeing Jr DS (Richmond, Canada), Quantiphi (Bengaluru), Renault (Tunis), TensorOps (EU), Wistron NeWeb (Taiwan), various Canada/UK jobright rows (Mekanism, Enercare, gunnercooke, etc.).
- **PhD-required / pure research:** Figma (DS Core Data PhD), NVIDIA (Research Scientist Efficient DL; Systems Perf; Deep Learning Computer Architect; GenAI Physical AI PhD), Citadel (Quant Researcher PhD), Snowflake AI Research Sci (already logged), DoorDash (AI Research Fellowship; SWE ML-infra), Proxima (AI Scientist ICML).
- **Senior / manager / staff:** Zoom (AI Eng / ML R17822 — seniority high), Salesforce (DE SMTS/LMTS Knowledge Graph & MDM = senior), HNTB (DE III), Comcast (DE 2/3), Mastercard (Mgr DE), Archer (Staff AI Research Eng), Vanguard (DE Specialist).
- **Internship (candidate graduates May 2026, weak timing) / co-op:** State Street BestX AI Eng (Jul–Dec 2026 internship), TD Bank (2026 Fall Co-op DE), Tesla intern roles (already logged), Wistron interns.
- **Wet-lab / clinical / non-data research:** the large block of jobright "Research Assistant/Coordinator/Clinical Data Abstractor/Statistical Programmer/Postdoc" rows (Fred Hutch, Mayo, Carta Healthcare, GeneDx, Eli Lilly, Pfizer Assoc Data Mgr, Thermo Fisher, Novartis, MSD, Caris, Flatiron, Emory, USC, Yale, etc.).
- **Non-target BA / generic non-technical analyst rows** (jobright DA tracker): Neuberger RFP Analyst, Baker Tilly Innovation Analyst, Gartner Research Analyst, Edelman/Annalect/Avōq/Real Chemistry agency analysts, Kharon/Exiger sanctions-research analysts, GIS/field-data-collector rows, NC Dept of Agriculture "analyst" rows, D.E. Shaw "ViduZZZi/AgneZZZ Test" placeholder rows (not real).
- **Generic large-co rows not individually listed** (seniority unverified, lower priority, captured so they aren't re-surfaced as "new"): Cisco DE (RTP ×2), CVS Mgr DE, AF Group / Emergent Holdings Assoc DA (Lansing MI), UL Solutions Assoc Business DA, CNO Financial Data & Reporting Analyst, LivaNova Data Analyst (incl Austin TX), DHL Supply Chain Business DA II, Pampered Chef Digital Insights Analyst.

### Bonus — adjacent titles worth adding to the search vocabulary
- **"Junior Data Services and AI Developer" / "AI Developer I"** (Citi, Western & Southern) — bank/insurer entry "AI Developer" rung; add alongside "AI Engineer."
- **"AI ___ Associate" cluster** — *AI Operations Associate* (Nevoya, Loop, Accordion), *AI Implementation Associate* (Highbeam), *Enterprise AI Associate* (Mercor), *AI Conversation Designer* (JustAnswer), *People Operations & AI Associate* (Solace), *AI Enablement Strategist* (GlobalFoundries), *AI Solutions Associate* (Digital United). High-volume this cycle but mostly **GTM/ops/adoption, not engineering** — keep but screen hard.
- **"Data Scientist I/1" / "Associate Data Engineer"** at non-FAANG enterprises (BofA, GumGum, Adswerve, P&G, CVS) — the most common entry rung touching this profile; confirm as standing search terms.
- **Texas Instruments "New College Graduate – AI Solutions Engineer"** — proof that a **DFW-local, exact-target-title NCG applied-AI role exists**; worth a recurring direct check of `careers.ti.com` (Richardson) each cycle even though the ATS 403s the fetcher and the current post predates the window.

### TODO for next (daily) run
- **Same two structural blockers persist (now 5 runs running) and remain the top priority:** (1) every ATS/career host (workday/greenhouse/ashby/oracle-`careers.ti.com`/smartrecruiters) 403s → all visa buckets stay inferred, never JD-verified; (2) every HN path (Algolia API, `hnhiring.com`, `nchelluri.github.io/hnjobs/`, startup.jobs) + Reddit 403/hard-block → community boards still uncovered. Need an authenticated/alternate fetch path for at least one of each before these can be trusted.
- **DFW-local direct check:** `careers.ti.com` (Richardson) is worth a manual/authenticated pull each cycle — it has exact-title NCG applied-AI roles but 403s the fetcher.
- **jobright DA** still caps at 7 days; **SimplifyJobs** AI/ML 37-role table still truncates — both need a non-README data path for true 30-day depth.
